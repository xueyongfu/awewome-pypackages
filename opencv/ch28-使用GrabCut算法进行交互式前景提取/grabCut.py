"""
img - 输入图像
• mask-掩模图像 用来确定 些区域是背景 前景 可能是前景/背景等。 可以 置为 cv2.GC_BGD,cv2.GC_FGD,cv2.GC_PR_BGD,cv2.GC_PR_FGD  
  或者直接 入 0,1,2,3 也 。
• iterCount - 算法的迭代次数
...
img = cv2.imread('ch28-使用GrabCut算法进行交互式前景提取/messi5.jpg')

mask = np.zeros(img.shape[:2], np.uint8)

# bdgModel, fgdModel - 算法内 使用的数组. 你只  创建两个大小为 (1,65) 数据类型为 np.float64 的数组。
