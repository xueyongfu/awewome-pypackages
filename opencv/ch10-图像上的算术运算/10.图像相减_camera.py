...
cv2.imshow('bgimg' + str(frame_no), bgimg)
...
while cap.isOpened():
    ...
    # cv2.findContours物体轮廓检测 有三个参数。第一个是输入图像，第二个是轮廓检索模式，第三个是轮廓近似方法。
    ...
    for cnt in contours:
        ...
        # cv2.aprroxPolyDP(cnt, epsilon， True)  # 用于获得轮廓的近似值
        # cnt为输入的轮廓值， epsilon为阈值T，通常使用轮廓的周长作为阈值，True表示的是轮廓是闭合的
